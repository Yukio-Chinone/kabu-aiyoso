...
<h4 id="searchTop" class="d-flex justify-content-start align-items-center mb-1">
    <span>お気に入り銘柄 / <?php echo $subTitle?></span>&nbsp;
...
<div class="rounded border bg-white p-3 mb-3 ">
    ...
    <?php if($status == 1){?>
        買い対象銘柄のうち「保有予定」または「保有中」の銘柄を管理します。
    <?php }?>
    <?php if($status == 2){?>
        買い対象銘柄のうち「様子見」または「検討中」の銘柄を管理します。
    <?php }?>
    <?php if($status == 3){?>
        空売り対象銘柄のうち「保有予定」または「保有中」の銘柄を管理します。
    <?php }?>
    <?php if($status == 4){?>
        空売り対象銘柄のうち「様子見」または「検討中」の銘柄を管理します。
    <?php }?>
    </div>
    ...
    <ul class="list-inline">
        ...
        <h3 class="mb-3" id="stock<?php echo $stockCode?>">
        ...
        <div class="row stock">
            ...
            <div class="col-md-6 mt-2">
                ...
                <div class="float2cloumn">
                    ...
                    <?php $targetTrainedYmd = ListedCompanies::getLatestTrainedDate($stockCode)?>
        ...
        <div class="chart_area">
            ...
            <div style="text-align:center;">
                <a href="javascript:void(0);" onclick='openClose("img_stock<?php echo $stockCode?>_<?php echo $searchResult->getYmd()?>", "img_yahoo_stock<?php echo $stockCode?>_<?php echo $searchResult->getYmd()?>");'>予想チャート</a>
                <span style="color:#CCC">&nbsp;|&nbsp;</span>
                <a href="javascript:void(0);" onclick='openClose("img_yahoo_stock<?php echo $stockCode?>_<?php echo $searchResult->getYmd()?>", "img_stock<?php echo $stockCode?>_<?php echo $searchResult->getYmd()?>");'>リアルタイムチャート(本日)</a>
            </div>
            <img width="100%"
                style="border:solid 1px #CCC;"
                class="trim_chart"
                id="img_stock<?php echo $stockCode?>_<?php echo $searchResult->getYmd()?>"
            ...
            <div style="padding:0px 9%;
                ...
                id="img_yahoo_stock<?php echo $stockCode?>_<?php echo $searchResult->getYmd()?>">
                <?php if($stockCode != Stock::$nikkei225){?>
            ...
            function go_forward_<?php echo $stockCode?>(day){
                ...
                document.getElementById("img_stock<?php echo $stockCode?>_<?php echo $searchResult->getYmd()?>").src=picsSrc_<?php echo $stockCode?>[num_<?php echo $stockCode?>];
            }
            ...
            <div class="day_buttons">
                &nbsp;&nbsp;
                <input id="ffbutton_<?php echo $stockCode?>" type="button" class="btn" value="<< 5日前" onclick="go_forward_<?php echo $stockCode?>(5)">
                <input id="fbutton_<?php echo $stockCode?>" type="button" class="btn" value="< 1日前" onclick="go_forward_<?php echo $stockCode?>(1)">
                &nbsp;&nbsp;
                <input id="bbutton_<?php echo $stockCode?>"type="button" class="btn" value="1日後 >" onclick="go_back_<?php echo $stockCode?>(1)" disabled="true">
                <input id="bbbutton_<?php echo $stockCode?>"type="button" class="btn" value="5日後 >>" onclick="go_back_<?php echo $stockCode?>(5)" disabled="true">
            </div>
            ...
            <a href="javascript:void(0);">
                <p id="imgbtn_stock<?php echo $stockCode?>_<?php echo $searchResult->getYmd()?>" onclick="biggerImg(<?php echo $stockCode?>, '<?php echo $searchResult->getYmd()?>')">+</p>
            </a>
        </div>
